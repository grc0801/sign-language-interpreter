...
def preprocess_keypoints(hand_list):
    ...
    if not hand_list:
        ...
        if len(hand_list) >= 2:
            # Sort hands by average x for consistent ordering.
...
ip_cam_url = 'http://100.72.107.139:8080/video'
...
while True:
    ...
    if hand_list:
        gesture, conf, _ = classify_gesture(hand_list)
        ...
        # If label has changed, print immediately.
        if smoothed != last_spoken_label:
            ...
            # If same label, update only if 3 seconds have passed.
